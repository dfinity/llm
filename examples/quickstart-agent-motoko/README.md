...
The first deploy creates fresh `agent-backend` and `agent-frontend` canisters
on mainnet; their principals are written to `.icp/data/mappings/ic.ids.json`,
which should be committed so future deploys reuse them. The `llm` canister is
excluded from the `ic` environment in `icp.yaml` — mainnet already runs the
canonical LLM canister at `w36hm-…` and the SDK addresses it directly.
